class Api::V1::ProfilesController < Api::ApiController
  ...
  def show
    ...
    if params[:schema].present?
      render :json => { :email => @user.email, :uid => @user.uid, :id => @user.uid } # continue support for deprecated :id field
      ...
      render :json => EMPTY_PROFILE.merge(:email => @user.email, :uid => @user.uid, :id => @user.uid) # continue support for deprecated :id field
    end
  end
end
