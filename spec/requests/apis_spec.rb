...
describe "Apis" do
  ...
  describe "POST /api/notifications" do
    ...
    before do
      ...
      login(@user)
      1.upto(14) do |index|
        @notification = Notification.create!({:subject => "Notification ##{index}", :received_at => Time.now - 1.hour, :body => "This is notification ##{index}.", :user_id => @user.id, :app_id => @app.id, :identifier => 'myapp-identifier', :delivery_type => 'text'}, :as => :admin)
      end
      @other_user_notification = Notification.create!({:subject => 'Other User Notification', :received_at => Time.now - 1.hour, :body => 'This is a notification for a different user.', :user_id => @other_user.id, :app_id => @app.id, :identifier => 'myuser-identifier', :delivery_type => 'text'}, :as => :admin)
      @other_app_notification = Notification.create!({:subject => 'Other App Notification', :received_at => Time.now - 1.hour, :body => 'This is a notification for a different app.', :user_id => @user.id, :app_id => @app2.id, :identifier => 'myapp2-identifier', :delivery_type => 'text'}, :as => :admin)
    ...
    context "when the user has a valid token" do
      context "when the notification attributes are valid" do
        it "should create a new notification when the notification info is valid" do
          ...
          post "/api/notifications", {:notification => {:subject => 'Project MyUSA', :body => 'This is a test.', :identifier => 'myapp-identifier', :delivery_type => 'text'}}, {'HTTP_AUTHORIZATION' => "Bearer #{@token}"}
          response.code.should == "200"
          @user.notifications.reload
